## Hi there 👋

I'm KingShedew! I specialize in making UBG and proxy-browser websites. Here are my current projects:
> (Note: Not all of my projects are in Github!)
* ECOG Infinite ([https://github.com/kingshedew/ecog](https://github.com/kingshedew/ecog)) - It's a UBG site with the best games, and more! It has a lot of exclusive things!
* Shedew-OS [UNFINISHED] ([https://github.com/KingShedew/shedew-os](https://github.com/KingShedew/shedew-os)) - A project that is supposed to look like an operating system, with it's own apps and more.
--- 
# Files For ECOG:
* ecog-files ([github.com/ecog-ent/ecog-files](https://github.com/ecog-ent/ecog-files)  - Just the game files used for ECOG.
* ecog-emulator ([https://github.com/ecog-ent/ecog-emulator](https://github.com/ecog-ent/ecog-emulator/)) - The emulator roms and zips used for ECOG.
Other repositories:
* [768394762]((https://github.com/KingShedew/768394762)) - A fixed version of Stealth Assassin.
* [SchoolHouseRemake](https://github.com/KingShedew/schoolhouseremake) - Code for Raldi's Cr@ckhouse that is supposed to be undetectable in Gaggle.
* [Eagler Hacks](https://github.com/KingShedew/eagler-hacks) - A bunch of hacked versions of EaglerCraft.
* [ECOG Snow Rider 3D](https://github.com/KingShedew/ecog-snow-rider-3d) - A fixed version of Snow Rider 3D. A fun game.
* [Gun Spin But It's Downloadable](https://github.com/KingShedew/gun-spin-but-its-downloadable) - A Gun Spin port that is able to be downloaded.
* [Getaway Shootout](https://github.com/KingShedew/getaway-shootout) - Getaway Shootout but it's downloadable.

